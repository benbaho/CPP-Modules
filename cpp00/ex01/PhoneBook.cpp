...
PhoneBook PhoneBook::add(PhoneBook phonebook)
{
    std::string name;
    std::string surname;
    std::string nickname;
...
void PhoneBook::search(PhoneBook phonebook, std::string array)
{
    int tmp = 0;
    ...
    for(int lim = 0; lim < 8 && !phonebook.book[lim].getName().empty();lim++)
    {
        ...
        if(is_all_numeric(array) && !array.empty())
        {
            ...
            if((tmp >= 0 && tmp < phonebook.c) && !array.empty())
            {
        ...
        {
            if(!array.empty())
                std::cout << "Error: Please enter [0 - 7]!"<< std::endl;
            goto control;
        }
}
...
int main()
{
    ...
    while(42)
    {
        ...
        if(!array.compare("ADD"))
            phonebook = phonebook.add(phonebook);
        else if(!array.compare("SEARCH") && !array.empty())
            phonebook.search(phonebook, array);
